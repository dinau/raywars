...
fn main() {
    ...
    for (i, line) in text_lines.iter().enumerate() {
        ...
        if text_width > 0 && text_height > 0 {
            let mut tex = rl
                .load_render_texture(&thread, text_width as u32, text_height as u32)
    ...
    let mut delay_show_window: i32 = 1;
    ...
    while !rl.window_should_close() {
        ...
        if delay_show_window == 0 {
            unsafe {
                ffi::ClearWindowState(ffi::ConfigFlags::FLAG_WINDOW_HIDDEN as u32);
            }
        }
        if delay_show_window >= 0 {
            delay_show_window -= 1;
